...
DATA_DIR = "/p/projects/impactee/Josh/WS_agg/data/"
OUTPUT_CSV = "/p/projects/impactee/Josh/WS_agg/output/aggregated_results.csv"
...
def process_all_rasters(gadm, data_dir):
    ...
    for raster_filename in os.listdir(data_dir):
        if raster_filename.endswith(".tif"):
            year_match = re.search(r'GDP(\d{4})\.tif', raster_filename)  # Match GDPYYYY.tif
            if not year_match:
                continue
